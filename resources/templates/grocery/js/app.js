/**
 * Indian Grocery template — storefront behaviour.
...
Alpine.store('ui', {
    ...
    locationOpen: false,
    packPicker: null, // product whose pack sizes are being chosen
    toast: null,
    toastTimer: null,
    get anyOpen() { return this.cartOpen || this.menuOpen || this.searchOpen || this.categoriesOpen || this.locationOpen || !!this.packPicker; },
    closeAll() { this.cartOpen = this.menuOpen = this.searchOpen = this.categoriesOpen = this.locationOpen = false; this.packPicker = null; },
    ...
    openCategories() { this.closeAll(); this.categoriesOpen = true; },
    openLocation() { this.closeAll(); this.locationOpen = true; },
    pickPack(product) { this.packPicker = product; },
...
Alpine.store('cart', {
    ...
    /** Total MRP savings across the bag (compare-at − price). */
    get savings() {
    ...
    line(productId, size = null) {
        return this.items.find((i) => i.product.id === productId && (size === null || i.size === size));
    },
    qtyOf(productId, size = null) {
        return this.items.filter((i) => i.product.id === productId && (size === null || i.size === size)).reduce((s, i) => s + i.qty, 0);
    },
...
Alpine.store('wishlist', {
    ids: Alpine.$persist([]).as('grocery-wishlist'),
    has(id) { return this.ids.includes(id); },
    toggle(id) { this.ids = this.has(id) ? this.ids.filter((x) => x !== id) : [id, ...this.ids]; Alpine.store('ui').notify(this.has(id) ? 'Saved to your list' : 'Removed from your list'); },
    get count() { return this.ids.length; },
});

Alpine.store('recent', {
    terms: Alpine.$persist([]).as('grocery-recent-searches'),
    add(term) { this.terms = [term, ...this.terms.filter((t) => t !== term)].slice(0, 6); },
    clear() { this.terms = []; },
});

Alpine.store('recent_products', {
    slugs: Alpine.$persist([]).as('grocery-recently-viewed'),
    push(slug) { this.slugs = [slug, ...this.slugs.filter((s) => s !== slug)].slice(0, 10); },
});

Alpine.store('location', {
    area: Alpine.$persist(null).as('grocery-delivery-area'),
    pincode: Alpine.$persist(null).as('grocery-delivery-pincode'),
    set(area, pincode = null) { this.area = area; this.pincode = pincode; Alpine.store('ui').closeAll(); Alpine.store('ui').notify(`Delivering to ${area}`); },
});

Alpine.store('cookies', {
    prefs: Alpine.$persist(null).as('grocery-cookie-consent'),
...
Alpine.data('search', (suggestions = []) => ({
    ...
    init() {
        ...
        setInterval(() => { this.placeholderIndex = (this.placeholderIndex + 1) % Math.max(1, this.suggestions.length); }, 2600);
        this.$watch('$store.ui.searchOpen', (o) => { if (o) this.$nextTick(() => this.$refs.input?.focus()); });
    },
    get placeholder() {
        return this.suggestions.length ? `Search "${this.suggestions[this.placeholderIndex]}"` : 'Search products…';
    },
    lookup() {
...
/** Product card with ADD → stepper behaviour synced to the server cart. */
Alpine.data('gCard', (product) => ({
    product,
    ...
    add() {
        ...
        if (this.multi) { Alpine.store('ui').pickPack(this.product); return; }
        Alpine.store('cart').inc(this.product.id, this.size, this.product.max_qty);
    },
    inc() { if (this.multi) { Alpine.store('ui').pickPack(this.product); return; } Alpine.store('cart').inc(this.product.id, this.size, this.product.max_qty); },
    dec() { if (this.multi) { Alpine.store('ui').pickPack(this.product); return; } Alpine.store('cart').dec(this.product.id, this.size); },
}));

/** Bottom sheet for choosing a pack size on multi-variant products. */
Alpine.data('packPicker', () => ({
    get p() { return Alpine.store('ui').packPicker; },
...
Alpine.data('heroCarousel', (count, interval = 5000) => ({
...
Alpine.data('shopFilters', (initial) => ({
    ...
    clear() {
        this.f = { category: [], subcategory: [], collection: [], brand: [], material: [], size: [], color: [], min: '', max: '', rating: '', availability: '', sale: false, veg: false, sort: this.f.sort, view: this.f.view };
        this.submit();
    },
    get activeCount() {
        let n = 0;
    ...
    submit() {
        ...
        for (const key of ['min', 'max', 'rating', 'availability', 'sort']) if (this.f[key] !== '' && this.f[key] != null && this.f[key] !== false) params.set(key, this.f[key]);
...
Alpine.data('productPage', (product) => ({
    ...
    add() { Alpine.store('cart').inc(this.p.id, this.size, this.p.max_qty); },
    dec() { Alpine.store('cart').dec(this.p.id, this.size); },
    async buyNow() {
        if (!this.qty) await Alpine.store('cart').add(this.p.id, this.size, null, 1, false);
...
Alpine.data('newsletter', (source = 'grocery') => ({
...
Alpine.data('locationPicker', () => ({
    pincode: '',
    error: null,
    areas: ['Bengaluru', 'Mumbai', 'Delhi NCR', 'Hyderabad', 'Chennai', 'Pune', 'Kolkata', 'Ahmedabad'],
    check() {
        if (!/^\d{6}$/.test(this.pincode)) { this.error = 'Enter a valid 6-digit pincode'; return; }
        this.error = null;
        Alpine.store('location').set(`PIN ${this.pincode}`, this.pincode);
    },
}));
