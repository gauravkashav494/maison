...
<div x-data x-show="$store.app.showInstall" x-cloak x-transition class="above-tabs px-3 pb-2 lg:hidden">
    <div class="flex items-center gap-3 border border-ink/10 bg-ivory p-3 shadow-[0_30px_60px_-30px_rgba(21,20,18,0.35)]">
        ...
        <div class="min-w-0 flex-1">
            <p class="font-serif text-base leading-tight">Get the {{ template()->pwa()['name'] ?? ($site['name'] ?? config('app.name')) }} app</p>
            <p class="mt-0.5 text-[0.6875rem] leading-snug text-smoke" x-text="$store.app.ios ? 'Tap Share, then “Add to Home Screen”.' : 'Full screen, faster checkout, always one tap away.'"></p>
        </div>
