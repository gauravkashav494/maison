@extends('layouts.app', ['appBar' => ['title' => 'Addresses', 'back' => true]])

@section('content')
<x-account-shell title="Service addresses">
    <div x-data="{ editing: {{ $errors->any() ? (old('_editing') ?: 'null') : 'null' }}, adding: {{ $errors->any() && !old('_editing') ? 'true' : 'false' }} }">
        <div class="grid gap-3 sm:grid-cols-2">
                ...
                <div class="card p-4">
                    <div x-show="editing !== {{ $a->id }}">
                        <div class="flex items-start justify-between gap-3">
                            <p class="flex items-center gap-2 font-display text-sm font-extrabold"><x-ico name="map-pin" :size="16" class="text-primary" /> {{ $a->label ?: 'Address' }}</p>
                            @if($a->is_default)<span class="pill pill-sky">Default</span>@endif
                        </div>
            ...
            @endforeach
            <div class="card border-2 border-dashed border-line shadow-none p-4">
                ...
                <form x-show="adding" x-cloak method="post" action="{{ route('account.addresses.store') }}" class="space-y-3">
                    @csrf
                    <p class="font-display text-sm font-extrabold">New address</p>
