    ...
    <div class="lg:col-span-7">
        ...
        @if($steps)
            <ol class="mt-4 grid gap-3 sm:grid-cols-3">
                @foreach($steps as $i => $s)<li class="card p-4"><span class="step-num">{{ $i + 1 }}</span><p class="mt-3 text-sm font-extrabold">{{ $s['title'] }}</p><p class="mt-1 text-xs leading-relaxed text-slate">{{ $s['text'] }}</p></li>@endforeach
            </ol>
        @endif
    </div>
    <div class="lg:col-span-5">
        @if($giftCard)
            <div class="card p-5 lg:sticky lg:top-[7.5rem]" x-data="{ amount: @js($giftCard->sizes[1] ?? $giftCard->sizes[0] ?? null) }">
                <p class="text-lg font-extrabold">{{ $giftCard->name }}</p>
                <p class="mt-1 text-sm text-slate">{{ $giftCard->description }}</p>
                ...
                <p class="mt-3 text-xs text-mist">Delivered by email within minutes. Valid for 12 months on every product.</p>
            </div>
        @else
