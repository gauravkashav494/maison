            ...
            @foreach($tables as $i => $t)
                <div x-show="tab === {{ $i }}" class="card overflow-x-auto p-2">
                    <table class="w-full text-sm">
                        <thead><tr class="bg-paper text-left text-xs uppercase tracking-wider text-slate">@foreach($t['columns'] ?? [] as $col)<th class="px-3 py-2 font-bold">{{ $col }}</th>@endforeach</tr></thead>
                        <tbody class="divide-y divide-line">@foreach($t['rows'] ?? [] as $row)<tr>@foreach((array) $row as $cell)<td class="px-3 py-2">{{ is_array($cell) ? implode(' ', $cell) : $cell }}</td>@endforeach</tr>@endforeach</tbody>
                    </table>
                </div>
            @endforeach
        @endif
