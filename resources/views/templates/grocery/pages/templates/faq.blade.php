...
<section class="g-container grid gap-6 py-6 lg:grid-cols-12" x-data="{ tab: @js($faqs->keys()->first()), q: '' }">
        ...
        <div class="card p-3 lg:sticky lg:top-[7.5rem]">
            ...
            <nav class="no-scrollbar mt-3 flex gap-1 overflow-x-auto lg:flex-col" aria-label="FAQ categories">
                @foreach($faqs as $cat => $items)
                    <button type="button" @click="tab = @js($cat); q = ''" class="shrink-0 rounded-lg px-3 py-2 text-left text-sm font-semibold" :class="tab === @js($cat) && !q ? 'bg-leaf-light text-leaf-dark' : 'text-slate hover:bg-paper'">{{ $cat }} <span class="text-xs text-mist">({{ $items->count() }})</span></button>
                @endforeach
            </nav>
        </div>
    </aside>
    <div class="lg:col-span-9">
        @if($page->body)<div class="prose-g mb-4 text-sm">{!! $page->body !!}</div>@endif
        @foreach($faqs as $cat => $items)
            <div x-show="q ? true : tab === @js($cat)" class="card mb-4 p-4 sm:p-5" x-data="accordion()">
                <h2 class="text-lg font-extrabold">{{ $cat }}</h2>
                <ul class="mt-2 divide-y divide-line">
                    @foreach($items as $f)
                        <li x-show="!q || @js(Str::lower($f->question.' '.strip_tags($f->answer))).includes(q.toLowerCase())">
                            ...
                            <div x-show="open === {{ $f->id }}" x-collapse x-cloak class="prose-g pb-4 text-sm">{!! $f->answer !!}</div>
                        </li>
                    @endforeach
                </ul>
            </div>
