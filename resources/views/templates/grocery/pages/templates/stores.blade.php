...
<section class="g-container py-6">
    @if($page->body)<div class="prose-g mb-6 max-w-3xl text-sm">{!! $page->body !!}</div>@endif
    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        @foreach($stores as $s)
            <div class="card overflow-hidden">
                <div class="aspect-[16/9] bg-leaf-light">@if($s->image_url)<img src="{{ $s->image_url }}" alt="" class="img-cover" loading="lazy">@endif</div>
                <div class="p-4">
                    <h2 class="text-lg font-extrabold">{{ $s->name }}</h2>
                    <p class="mt-1 text-sm text-slate">{{ $s->address }}, {{ $s->city }} {{ $s->postal_code }}</p>
                    @if($s->hours)<p class="mt-2 whitespace-pre-line text-xs text-slate"><x-ico name="clock" :size="12" class="mr-1 inline text-leaf" />{{ $s->hours }}</p>@endif
