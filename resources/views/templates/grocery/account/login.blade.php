...
<x-auth-shell title="Welcome back" text="Log in to see your orders, addresses and saved items.">
    <form method="post" action="{{ route('login') }}" class="space-y-4">
        ...
        <div class="flex items-center justify-between text-sm">
            ...
            <a href="{{ route('password.request') }}" class="font-semibold text-leaf hover:underline">Forgot password?</a>
        </div>
        <button type="submit" class="btn btn-primary btn-lg btn-block">Log in</button>
    </form>
    <p class="mt-5 text-center text-sm text-slate">New here? <a href="{{ route('register') }}" class="font-bold text-leaf hover:underline">Create an account</a></p>
</x-auth-shell>
@endsection
