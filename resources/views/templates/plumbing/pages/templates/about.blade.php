...
<section class="p-container space-y-8 py-6">
    ...
    @foreach($d['sections'] ?? [] as $i => $s)
        ...
        <div class="card grid overflow-hidden lg:grid-cols-2">
            <div class="{{ $flip ? 'lg:order-2' : '' }} p-6 sm:p-8 lg:p-10">
                @if(!empty($s['eyebrow']))<p class="text-xs font-bold uppercase tracking-wider text-primary">{{ $s['eyebrow'] }}</p>@endif
                <h2 class="mt-1 font-display text-2xl font-bold">{{ $s['heading'] ?? '' }}</h2>
                <div class="prose-p mt-3 text-sm">{!! $s['body'] ?? '' !!}</div>
            </div>
            <div class="min-h-56 bg-sky {{ $flip ? 'lg:order-1' : '' }}">@if($img)<img src="{{ $img }}" alt="" class="img-cover" loading="lazy">@endif</div>
        </div>
    @endforeach
    ...
    @if(!empty($d['sustainability_heading']))
        <div class="grid overflow-hidden rounded-2xl bg-primary text-white lg:grid-cols-2">
            <div class="p-6 sm:p-8 lg:p-10"><p class="text-xs font-bold uppercase tracking-wider text-white/80">Sustainability</p><h2 class="mt-1 font-display text-2xl font-bold">{{ $d['sustainability_heading'] }}</h2><div class="prose-p mt-3 text-sm text-white/85 [&_*]:text-white/85">{!! $d['sustainability_body'] ?? '' !!}</div></div>
