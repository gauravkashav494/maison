...
<section class="p-container grid gap-6 py-6 lg:grid-cols-12">
    <div class="lg:col-span-8">
        ...
        @if($page->body)<div class="prose-p mb-6 text-sm">{!! $page->body !!}</div>@endif
        <h2 class="font-display text-lg font-bold">Open roles</h2>
        @if(empty($roles))
    ...
    <aside class="lg:col-span-4">
        @if($perks)
            <div class="card p-5"><p class="text-sm font-extrabold">Why work with us</p><ul class="mt-3 space-y-2 text-sm text-slate">@foreach($perks as $perk)<li class="flex gap-2"><x-ico name="check" :size="16" class="mt-0.5 shrink-0 text-primary" /> {{ $perk }}</li>@endforeach</ul></div>
        @endif
    </aside>
</section>
@endsection
