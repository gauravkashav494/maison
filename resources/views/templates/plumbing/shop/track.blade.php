    ...
    <div class="mx-auto max-w-3xl space-y-4">
            ...
            <div class="card p-4 sm:p-5">
                <div class="flex flex-wrap items-start justify-between gap-3">
                    <div>
                        ...
                        <p class="mt-1 font-display text-lg font-bold">{{ $order->statusLabel() }}</p>
                    ...
                    @if($order->tracking_number)<div class="rounded-lg bg-canvas px-3 py-2 text-xs"><span class="text-slate">{{ $order->carrier ?: 'Tracking' }}</span><span class="ml-2 font-bold tabular">{{ $order->tracking_number }}</span></div>@endif
                </div>
        ...
        @else
            <div class="grid gap-3 sm:grid-cols-3">
                <div class="card p-4 text-sm"><x-ico name="receipt" :size="20" class="text-primary" /><p class="mt-2 font-bold">Where’s my order number?</p><p class="text-xs text-slate">It’s in your confirmation email and on the order page, e.g. ME-2026-00123.</p></div>
                <div class="card p-4 text-sm"><x-ico name="user" :size="20" class="text-primary" /><p class="mt-2 font-bold">Have an account?</p><p class="text-xs text-slate"><a href="{{ route('account.orders') }}" class="font-semibold text-primary">Log in</a> to see every order and its live status.</p></div>
                <div class="card p-4 text-sm"><x-ico name="phone" :size="20" class="text-primary" /><p class="mt-2 font-bold">Need help?</p><p class="text-xs text-slate"><a href="/contact" class="font-semibold text-primary">Contact support</a> — we reply within a few hours.</p></div>
            </div>
        @endif
    </div>
</section>
