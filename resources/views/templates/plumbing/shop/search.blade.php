...
<x-page-head :title="$q ? 'Results for “'.$q.'”' : 'Search'" :text="$q ? $products->total().' '.Str::plural('product', $products->total()).' found' : 'Search by product, size (1 inch), material (CPVC) or brand.'" :breadcrumbs="['Search' => null]" />
...
<section class="p-container py-6">
    ...
    @elseif($products->isEmpty())
        <x-empty-state class="mx-auto mt-8 max-w-2xl" icon="search" title="No results for “{{ $q }}”" text="Check the spelling or try a broader term like “elbow”, “mixer” or “valve”.">
