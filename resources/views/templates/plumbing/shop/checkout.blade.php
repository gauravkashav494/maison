...
<section class="p-container py-6" x-data="checkout({ shipping: @js($old('shipping_method', $cart['shipping_method']['code'])), payment: @js($old('payment_method', array_key_first($paymentMethods))), codFee: {{ $codFee }} })">
    @if($errors->any())
        <div class="mb-5 flex items-start gap-2 rounded-xl border border-danger/30 bg-danger-light px-4 py-3 text-sm text-danger"><x-ico name="alert" :size="18" class="mt-0.5 shrink-0" /> Please check the highlighted fields below.</div>
    @endif
    ...
    <form method="post" action="{{ route('checkout.place') }}" class="grid gap-6 lg:grid-cols-12 lg:gap-8" @submit="submitting = true">
        ...
        <div class="space-y-4 lg:col-span-8">
            ...
            <fieldset class="card p-4 sm:p-5">
                ...
                @guest<p class="mt-2 text-xs text-slate">Have an account? <a href="{{ route('login') }}?redirect=checkout" class="font-semibold text-primary underline underline-offset-2">Log in</a> to use your saved addresses and GST details.</p>@endguest
            ...
            <fieldset class="card p-4 sm:p-5">
                ...
                <div class="mt-4 grid gap-2">
                    @foreach($shippingMethods as $m)
                        ...
                        <label class="flex cursor-pointer items-start gap-3 rounded-xl border px-4 py-3 transition-colors" :class="shipping === @js($m['code']) ? 'border-primary bg-sky/60' : 'border-line hover:border-mist'">
                            ...
                            <span class="flex-1">
                                <span class="flex items-baseline justify-between gap-3"><span class="text-sm font-bold">{{ $m['name'] }}</span><span class="text-sm font-bold tabular {{ ($free || $m['cost'] === 0) ? 'text-primary' : '' }}">{{ ($free || $m['cost'] === 0) ? 'FREE' : money($m['cost']) }}</span></span>
            ...
            <fieldset class="card p-4 sm:p-5">
                ...
                <div class="mt-4 grid gap-2">
                    @foreach($paymentMethods as $code => $name)
                        <div class="rounded-xl border transition-colors" :class="payment === @js($code) ? 'border-primary' : 'border-line'">
                            ...
                            <div x-show="payment === @js($code)" x-collapse x-cloak class="border-t border-line bg-canvas px-4 py-4 text-sm">
                                @switch($code)
                                    @case('card')
            ...
            <label class="card block p-4 sm:p-5"><span class="label">Order notes — GSTIN, site contact, delivery instructions (optional)</span><textarea name="notes" rows="2" class="field" placeholder="e.g. GSTIN 07AAAAA0000A1Z5 · Site contact: Ramesh 98xxxxxx · Deliver after 10 am">{{ $old('notes') }}</textarea></label>
        </div>
        ...
        <aside class="lg:col-span-4">
            <div class="card p-4 sm:p-5 lg:sticky lg:top-[7.5rem]">
                ...
                <ul class="mt-3 max-h-64 divide-y divide-line overflow-y-auto">
                    @foreach($cart['items'] as $item)
                        <li class="flex items-center gap-3 py-2.5">
                            <div class="relative h-12 w-12 shrink-0 overflow-hidden rounded-lg border border-line bg-canvas"><img src="{{ $item['product']['images'][0] ?? '' }}" alt="" class="img-cover"><span class="absolute -right-0 -top-0 grid h-4 min-w-4 place-items-center rounded-bl-md bg-ink px-1 text-[0.625rem] font-bold text-white">{{ $item['qty'] }}</span></div>
                ...
                <dl class="mt-3 space-y-2 border-t border-line pt-3 text-sm" x-data>
                    ...
                    <div class="flex justify-between" x-show="$store.cart.discount > 0"><dt class="text-slate">Coupon <span x-text="$store.cart.coupon ? '(' + $store.cart.coupon.code + ')' : ''"></span></dt><dd class="tabular text-primary" x-text="'− ' + $store.cart.discount_formatted"></dd></div>
