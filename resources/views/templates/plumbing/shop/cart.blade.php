...
<section class="p-container pb-28 pt-4 lg:py-6" x-data="{ code: '' }">
    ...
    <div x-show="$store.cart.items.length" x-cloak class="above-tabs border-t border-line bg-white p-3 shadow-float lg:hidden">
        <div class="flex items-center gap-3">
            <div class="min-w-0 flex-1"><p class="text-xs text-slate">To pay</p><p class="font-display text-lg font-bold tabular" x-text="$store.cart.total_formatted"></p></div>
    ...
    <div x-show="$store.cart.loaded && !$store.cart.items.length" x-cloak>
        <x-empty-state icon="cart" title="Your cart is empty" text="Pipes, fittings, taps, valves — add what the job needs and check out in minutes.">
    ...
    <div x-show="$store.cart.items.length" x-cloak class="grid gap-6 lg:grid-cols-12 lg:gap-8">
        <div class="space-y-4 lg:col-span-8">
            ...
            <div class="card p-4" x-show="$store.cart.free_shipping_threshold > 0">
                ...
                <div class="mt-2 h-2 overflow-hidden rounded-full bg-line"><div class="h-full rounded-full bg-primary transition-all duration-500" :style="`width:${Math.min(100, ($store.cart.progress || 0) * 100)}%`"></div></div>
            </div>

            <div class="card divide-y divide-line">
                <div class="flex items-center justify-between px-4 py-3">
                    <p class="text-sm font-extrabold">Items <span class="font-semibold text-slate" x-text="'(' + $store.cart.count + ')'"></span></p>
                    <p class="text-xs font-semibold text-primary" x-show="$store.cart.savings > 0"><x-ico name="sparkle" :size="12" class="inline" /> Saving <span x-text="$store.cart.savings_formatted"></span></p>
                </div>
                <template x-for="item in $store.cart.items" :key="item.key">
                    <div class="flex gap-4 px-4 py-4">
                        <a :href="item.product.url" class="h-20 w-20 shrink-0 overflow-hidden rounded-lg border border-line bg-canvas sm:h-24 sm:w-24"><img :src="item.product.images[0]" :alt="item.product.name" class="img-cover"></a>
                        <div class="min-w-0 flex-1">
                            <div class="flex items-start justify-between gap-3">
                                <div class="min-w-0">
                                    <p class="text-xs font-semibold text-slate" x-text="item.product.brand || item.product.category"></p>
                                    <a :href="item.product.url" class="line-clamp-2 text-sm font-bold leading-snug hover:text-primary" x-text="item.product.name"></a>
                                    <p class="mt-0.5 text-xs text-slate" x-text="item.size"></p>
                                </div>
                                <button type="button" @click="$store.cart.remove(item.key)" class="grid h-8 w-8 shrink-0 place-items-center rounded-lg text-mist hover:bg-canvas hover:text-danger" aria-label="Remove"><x-ico name="trash" :size="16" /></button>
                            </div>
                            <div class="mt-2 flex items-center justify-between gap-3">
                                <div class="flex items-baseline gap-2"><span class="text-base font-extrabold tabular" x-text="item.line_total_formatted"></span><span x-show="item.product.compare_at_price" class="text-xs text-mist line-through" x-text="'₹' + (item.product.compare_at_price * item.qty).toLocaleString('en-IN')"></span><span class="text-xs text-slate" x-text="item.qty > 1 ? item.product.price_formatted + ' each' : ''"></span></div>
                                <div class="stepper">
        ...
        <aside class="lg:col-span-4">
            <div class="space-y-4 lg:sticky lg:top-[7.5rem]">
                <div class="card p-4">
                    ...
                    <div x-show="$store.cart.coupon" x-cloak class="mt-2 flex items-center justify-between rounded-lg bg-sky px-3 py-2 text-sm">
                        <span class="font-bold text-primary"><span x-text="$store.cart.coupon?.code"></span> applied <span class="font-normal" x-text="'· ' + ($store.cart.coupon?.label || '')"></span></span>
                        <button type="button" @click="$store.cart.removeCoupon()" class="text-xs font-semibold text-slate hover:text-danger">Remove</button>
                    </div>
                </div>
                ...
                <div class="card p-4">
                    ...
                    <dl class="mt-3 space-y-2 text-sm">
                        ...
                        <div class="flex justify-between" x-show="$store.cart.discount > 0"><dt class="text-slate">Coupon discount</dt><dd class="tabular text-primary" x-text="'− ' + $store.cart.discount_formatted"></dd></div>
                        <div class="flex justify-between"><dt class="text-slate">Delivery charge</dt><dd class="tabular" :class="$store.cart.shipping === 0 && 'font-bold text-primary'" x-text="$store.cart.shipping === 0 ? 'FREE' : $store.cart.shipping_formatted"></dd></div>
