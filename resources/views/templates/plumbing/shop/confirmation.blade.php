...
@section('content')
<section class="p-container py-8 lg:py-12">
    <div class="mx-auto max-w-3xl">
        <div class="card p-6 text-center sm:p-8">
            <span class="mx-auto grid h-16 w-16 place-items-center rounded-full bg-sky text-primary"><x-ico name="check" :size="32" :stroke="3" /></span>
            <h1 class="mt-4 font-display text-2xl font-bold sm:text-3xl">Order placed! 🎉</h1>
            ...
            @if($order->estimated_delivery)<p class="mt-4 inline-flex items-center gap-2 rounded-full bg-accent-light px-4 py-1.5 text-sm font-bold text-accent-dark"><x-ico name="clock" :size="16" /> Arriving by {{ $order->estimated_delivery->format('l, d M') }}</p>@endif
        ...
        @guest
            <div class="card mt-4 flex flex-col items-center gap-3 p-5 text-center sm:flex-row sm:text-left">
                <span class="grid h-12 w-12 shrink-0 place-items-center rounded-full bg-sky text-primary"><x-ico name="user" :size="22" /></span>
