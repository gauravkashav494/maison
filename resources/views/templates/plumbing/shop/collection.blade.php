...
<div class="p-container hidden pt-4 lg:block"><x-breadcrumbs :items="['Project kits' => route('collections.index'), $collection->name => null]" /></div>

<section class="p-container py-4 lg:py-6">
    <div class="relative overflow-hidden rounded-2xl bg-primary text-white">
        ...
        <div class="relative p-6 sm:p-10 lg:max-w-2xl lg:p-12">
            ...
            <h1 class="mt-1 font-display text-2xl font-bold leading-tight sm:text-4xl">{{ $collection->name }}</h1>
            ...
            <div class="mt-4 flex flex-wrap items-center gap-3 text-sm">
                ...
                <span class="rounded-full bg-white/15 px-3 py-1 font-semibold">Kit value {{ money($total) }}@if($mrp > $total) <span class="text-white/70 line-through">{{ money($mrp) }}</span>@endif</span>
            </div>
        </div>
    </div>

    @if($collection->body)<div class="prose-p mx-auto mt-8 max-w-3xl text-sm">{!! $collection->body !!}</div>@endif
    ...
    @if($related->isNotEmpty())
        <div class="mt-10">
            <x-section-head title="More project kits" :href="route('collections.index')" />
            <div class="grid gap-3 sm:grid-cols-3">
                @foreach($related as $c)
                    <a href="{{ $c->url }}" class="card card-hover flex items-center gap-3 p-3">
                        <span class="h-16 w-16 shrink-0 overflow-hidden rounded-lg bg-sky">@if($c->image_url)<img src="{{ $c->image_url }}" alt="" class="img-cover" loading="lazy">@endif</span>
