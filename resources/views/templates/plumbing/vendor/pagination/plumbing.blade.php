                ...
                @if (is_array($element))
                    @foreach ($element as $page => $url)
                        @if ($page == $paginator->currentPage())
                            <span class="grid h-9 w-9 place-items-center rounded-lg bg-primary text-sm font-bold text-white" aria-current="page">{{ $page }}</span>
                        @else
                            <a href="{{ $url }}" class="grid h-9 w-9 place-items-center rounded-lg text-sm font-semibold hover:bg-sky">{{ $page }}</a>
                        @endif
                    @endforeach
                @endif
            @endforeach
