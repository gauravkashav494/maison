{{-- App-style bottom tabs (phones/tablets): Home · Categories/Filter · Search · Wishlist · Account --}}
@php
    $tab = fn (bool $on) => 'tab-item '.($on ? 'is-active text-primary' : 'text-ink');
...
<nav x-data class="tab-bar border-t border-line bg-white/95 backdrop-blur lg:hidden" aria-label="Quick navigation">
    <ul class="grid grid-cols-5">
        ...
        @else
            <li><button type="button" @click="$store.ui.openMenu()" class="{{ $tab($onShop) }}"><span class="tab-ico"><x-ico name="grid" :size="18" :stroke="1.9" /></span>Categories</button></li>
        @endif
        ...
        <li><a href="{{ route('account.wishlist') }}" class="{{ $tab(request()->is('account/wishlist')) }}"><span class="tab-ico"><x-ico name="heart" :size="18" :stroke="1.9" /></span>Wishlist<span x-show="$store.wishlist.count" x-cloak class="tab-badge bg-accent text-ink" x-text="$store.wishlist.count"></span></a></li>
