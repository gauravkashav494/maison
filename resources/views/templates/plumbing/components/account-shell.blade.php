...
@php
    $nav = [
        ...
        ['Wishlist', route('account.wishlist'), 'account/wishlist', 'heart'],
...
<section class="p-container grid gap-6 py-6 lg:grid-cols-12 lg:gap-8">
    <aside class="min-w-0 lg:col-span-3">
        <nav class="card no-scrollbar flex gap-1 overflow-x-auto p-2 lg:flex-col" aria-label="Account">
            @foreach($nav as [$label, $url, $pattern, $icon])
                <a href="{{ $url }}" class="flex shrink-0 items-center gap-2.5 rounded-lg px-3 py-2.5 text-sm font-semibold {{ request()->is($pattern) ? 'bg-sky text-primary' : 'text-slate hover:bg-canvas hover:text-ink' }}"><x-ico :name="$icon" :size="18" /> {{ $label }}</a>
            @endforeach
            @auth
                <form method="post" action="{{ route('logout') }}" class="shrink-0 lg:mt-2 lg:border-t lg:border-line lg:pt-2">@csrf<button type="submit" class="flex w-full items-center gap-2.5 rounded-lg px-3 py-2.5 text-sm font-semibold text-slate hover:bg-canvas hover:text-danger"><x-ico name="logout" :size="18" /> Log out</button></form>
            @endauth
        </nav>
    </aside>
