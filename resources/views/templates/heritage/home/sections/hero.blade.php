...
<section class="h-container pt-4 lg:pt-6" x-data="heroCarousel({{ count($slides) }})" @mouseenter="paused = true" @mouseleave="paused = false" aria-roledescription="carousel">
    <div class="banner-round relative bg-cream-dark">
        @foreach($slides as $i => $s)
            ...
            <div x-show="index === {{ $i }}" x-transition:enter="transition duration-700" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" class="relative min-h-[22rem] sm:min-h-[24rem] lg:min-h-[28rem]" @if($i > 0) x-cloak @endif>
                {{-- Full-bleed photo with a soft cream wash on the left so the copy stays legible --}}
                ...
                <div class="relative z-10 flex h-full min-h-[22rem] max-w-xl flex-col justify-center p-6 pb-10 sm:p-10 lg:min-h-[28rem] lg:max-w-3xl lg:p-14">
