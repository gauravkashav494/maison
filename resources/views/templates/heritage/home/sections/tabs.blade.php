...
@if($tabs->isNotEmpty())
<section class="h-container py-10 lg:py-[60px]" x-data="{ tab: 0 }">
    <h2 class="title-c lg:leading-none">{{ $title }}</h2>
    <div class="no-scrollbar mt-6 overflow-x-auto pb-1 lg:mt-[33px] lg:pb-[49px]"><div class="mx-auto flex w-max max-w-none gap-2.5">
    ...
    @foreach($tabs as $i => $t)
        ...
        <div x-show="tab === {{ $i }}" class="mt-6 lg:mt-0" @if($i > 0) x-cloak @endif>
            <div x-data="rail()" class="relative">
                <div x-ref="track" class="rail no-scrollbar -mx-4 px-4 sm:mx-0 sm:px-0 lg:gap-2.5">
                    <a href="{{ $url }}" class="tile-card w-[11.5rem] sm:w-[13.5rem] lg:mr-2 lg:w-[268px]" data-slide>
                    ...
                    @foreach($t['products'] as $product)<x-product-card :product="$product" :compact="true" class="pcard-tall lg:!w-[244px]" />@endforeach
                </div>
            ...
            <p class="text-center lg:hidden"><a href="{{ $url }}" class="view-all">View All</a></p>
        </div>
    @endforeach
</section>
@endif
