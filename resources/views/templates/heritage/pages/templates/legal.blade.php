    ...
    @if($headings)
        <aside class="lg:col-span-3">
            <nav class="card p-4 lg:sticky lg:top-[7.5rem]" aria-label="On this page">
                ...
                <ol class="mt-2 space-y-1.5 text-sm">@foreach($headings as [$id, $label])<li><a href="#{{ $id }}" class="text-muted hover:text-red">{{ $label }}</a></li>@endforeach</ol>
            </nav>
        </aside>
    @endif
    <div class="{{ $headings ? 'lg:col-span-9' : 'lg:col-span-8 lg:col-start-3' }}">
        <div class="card prose-h p-5 sm:p-8">
            <p class="!mt-0 text-xs text-muted">Last updated {{ $page->updated_at->format('d M Y') }}</p>
            {!! $body !!}
        </div>
    </div>
</section>
