    ...
    <div class="mx-auto max-w-4xl space-y-4">
        ...
        @foreach($page->data['materials'] ?? [] as $m)
            ...
            <div class="card grid overflow-hidden sm:grid-cols-[12rem_1fr]">
                <div class="min-h-40 bg-cream">@if($img)<img src="{{ $img }}" alt="" class="img-cover" loading="lazy">@endif</div>
                <div class="p-5">
                    <h2 class="text-lg font-semibold">{{ $m['name'] ?? '' }}</h2>
                    <ul class="mt-2 space-y-1.5 text-sm text-muted">@foreach($m['tips'] ?? [] as $tip)<li class="flex gap-2"><x-ico name="check" :size="16" class="mt-0.5 shrink-0 text-red" /> {{ $tip }}</li>@endforeach</ul>
                </div>
            </div>
        @endforeach
    </div>
