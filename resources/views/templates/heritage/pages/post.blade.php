...
<article class="h-container py-6">
    <div class="mx-auto max-w-3xl">
        ...
        <h1 class="mt-2 text-3xl font-semibold leading-tight sm:text-4xl">{{ $post->title }}</h1>
        @if($post->excerpt)<p class="mt-3 text-base text-muted">{{ $post->excerpt }}</p>@endif
        @if($post->image_url)<img src="{{ $post->image_url }}" alt="" class="mt-6 aspect-[16/9] w-full rounded-2xl object-cover">@endif
        <div class="prose-h mt-8">{!! $post->body !!}</div>
    </div>
    @if($more->isNotEmpty())
        <div class="mx-auto mt-12 max-w-3xl">
            ...
            <div class="grid gap-3 sm:grid-cols-2">
                @foreach($more as $p)
                    <a href="{{ $p->url }}" class="card card-hover flex items-center gap-3 p-3"><span class="h-16 w-16 shrink-0 overflow-hidden rounded-lg bg-cream">@if($p->image_url)<img src="{{ $p->image_url }}" alt="" class="img-cover" loading="lazy">@endif</span><span><span class="block text-sm font-bold">{{ $p->title }}</span><span class="line-clamp-2 text-xs text-muted">{{ $p->excerpt }}</span></span></a>
                @endforeach
            </div>
        </div>
    @endif
