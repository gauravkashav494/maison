...
<header x-data="header()" class="app-bar sticky top-0 z-50 bg-warm transition-shadow" :class="scrolled && 'shadow-[0_6px_24px_-16px_rgba(41,35,31,.45)]'">
    ...
    <div class="h-container flex h-14 items-center gap-2 lg:h-[87px] lg:gap-6">
        ...
        <a href="{{ route('home') }}" class="flex shrink-0 items-center gap-2 lg:gap-3" aria-label="{{ $site['name'] ?? config('app.name') }} — home">
            <span class="grid h-10 w-10 place-items-center rounded-full border-2 border-gold bg-cream text-red lg:h-[85px] lg:w-[85px]">
                <span class="flex flex-col items-center leading-none"><x-ico name="diamond" :size="14" :stroke="1.8" class="max-lg:hidden" /><x-ico name="diamond" :size="20" :stroke="1.6" class="lg:hidden" /><span class="mt-1 hidden font-serif text-[0.5rem] font-semibold uppercase tracking-[0.1em] lg:flex lg:text-[0.7rem]">{{ Str::substr($h['logo_primary'] ?? 'Annapurna', 0, 9) }}</span><span class="hidden text-[0.42rem] uppercase tracking-[0.2em] text-gold lg:block lg:text-[0.55rem]">{{ $h['logo_sub'] ?? '' }}</span></span>
            </span>
            <span class="flex flex-col font-serif leading-none lg:hidden {{ request()->routeIs('home') ? '' : 'max-sm:hidden' }}"><span class="text-[1.05rem] font-semibold tracking-tight text-red sm:text-[1.25rem]">{{ $h['logo_primary'] ?? 'Annapurna' }}</span><span class="mt-0.5 text-[0.8rem] italic text-gold sm:text-[0.95rem]">{{ $h['logo_accent'] ?? '' }}</span></span>
        </a>

        <div class="hidden flex-1 lg:block">
            <div class="mx-auto max-w-[688px]">@include('partials.search-box')</div>
        </div>

        <div class="ml-auto flex items-center">
            <button type="button" @click="$store.ui.openSearch()" class="grid h-11 w-11 place-items-center rounded-lg hover:bg-cream lg:hidden" aria-label="Search"><x-ico name="search" :size="22" /></button>
            <a href="{{ auth()->check() ? route('account.index') : route('login') }}" class="hidden h-11 w-11 place-items-center rounded-lg text-red hover:bg-cream sm:grid" aria-label="Account" title="{{ auth()->check() ? 'My account' : 'Login' }}"><x-ico name="user" :size="26" /></a>
            <a href="{{ route('track') }}" class="hidden h-11 w-11 place-items-center rounded-lg text-red hover:bg-cream sm:grid" aria-label="Track order" title="Track order"><x-ico name="truck" :size="28" /></a>
            <a href="{{ route('account.wishlist') }}" class="relative hidden h-11 w-11 place-items-center rounded-lg text-red hover:bg-cream md:grid" aria-label="Wishlist" title="Wishlist"><x-ico name="heart" :size="26" /><span x-show="$store.wishlist.count" x-cloak class="absolute right-0.5 top-0.5 grid h-4 min-w-4 place-items-center rounded-full bg-gold px-1 text-[0.625rem] font-bold text-maroon-deep" x-text="$store.wishlist.count"></span></a>
            <button type="button" @click="$store.ui.openCart()" class="relative grid h-11 w-11 place-items-center rounded-lg text-red hover:bg-cream" aria-label="Open cart" title="Cart"><x-ico name="bag" :size="26" /><span x-show="$store.cart.count" x-cloak class="absolute right-0 top-0 grid h-4 min-w-4 place-items-center rounded-full bg-red px-1 text-[0.625rem] font-bold text-white" x-text="$store.cart.count"></span></button>
        </div>
    </div>
    ...
    <nav class="hidden border-t border-line-soft bg-cream/60 lg:block" aria-label="Main navigation" @mouseleave="scheduleClose()">
        <div class="h-container flex items-center justify-center">
