                ...
                <div class="card p-4">
                    <div x-show="editing !== {{ $a->id }}">
                        <div class="flex items-start justify-between gap-3">
                            <p class="flex items-center gap-2 text-sm font-semibold"><x-ico name="pin" :size="16" class="text-red" /> {{ $a->label ?: 'Address' }}</p>
                            @if($a->is_default)<span class="badge badge-soft">Default</span>@endif
                        </div>
                        <div class="mt-2 text-sm leading-relaxed text-muted">@foreach($a->lines() as $l)<p>{{ $l }}</p>@endforeach</div>
                        <div class="mt-3 flex gap-2">
                            <button type="button" @click="editing = {{ $a->id }}" class="btn btn-ghost btn-sm"><x-ico name="edit" :size="14" /> Edit</button>
                            <form method="post" action="{{ route('account.addresses.destroy', $a) }}" onsubmit="return confirm('Remove this address?')">@csrf @method('DELETE')<button type="submit" class="btn btn-ghost btn-sm text-red"><x-ico name="trash" :size="14" /> Remove</button></form>
                        </div>
                    </div>
