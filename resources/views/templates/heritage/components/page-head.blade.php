...
<div class="relative overflow-hidden border-b border-line bg-warm">
    @if($image)<img src="{{ $image }}" alt="" class="absolute inset-0 h-full w-full object-cover opacity-20">@endif
    <div class="h-container relative py-8 lg:py-12">
        <x-breadcrumbs :items="$breadcrumbs" />
        @if($eyebrow)<p class="eyebrow mt-4">{{ $eyebrow }}</p>@endif
        <h1 class="display mt-2 text-3xl text-maroon lg:text-[2.75rem]">{{ $title }}</h1>
