...
<article {{ $attributes->class(['pcard', 'w-[11.5rem] sm:w-[13.5rem] lg:w-[15.25rem]' => $compact]) }} x-data="hCard(@js($card))" data-slide>
    <div class="pcard-media">
        ...
        <button type="button" @click="$store.wishlist.toggle(product.id)" :aria-pressed="$store.wishlist.has(product.id)" class="absolute right-2.5 top-2.5 grid h-8 w-8 place-items-center rounded-full bg-white/95 text-muted shadow-sm hover:text-red" :class="$store.wishlist.has(product.id) && '!text-red'" aria-label="Add to wishlist"><x-ico name="heart" :size="15" ::class="$store.wishlist.has(product.id) && 'fill-current'" /></button>
        <button type="button" @click="$store.ui.showQuickView(product.slug)" class="absolute bottom-2.5 right-2.5 hidden h-8 w-8 place-items-center rounded-full bg-white/95 text-muted shadow-sm hover:text-red lg:grid" aria-label="Quick view"><x-ico name="eye" :size="15" /></button>
    ...
    <div class="pcard-body">
        ...
        @if($product->review_count > 0)<p class="-mt-1 flex items-center justify-center gap-1 text-[0.6875rem] text-muted"><x-ico name="star" :size="11" class="fill-star text-star" /> {{ number_format($product->rating, 1) }} ({{ number_format($product->review_count) }})</p>@endif
