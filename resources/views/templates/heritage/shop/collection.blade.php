...
<section class="h-container py-4 lg:py-6">
    <div class="relative overflow-hidden rounded-2xl bg-red text-white">
        ...
        <div class="relative p-6 sm:p-10 lg:max-w-2xl lg:p-12">
            ...
            <h1 class="mt-1 text-2xl font-semibold leading-tight sm:text-4xl">{{ $collection->name }}</h1>
            ...
            <div class="mt-4 flex flex-wrap items-center gap-3 text-sm">
                ...
                <span class="rounded-full bg-white/15 px-3 py-1 font-semibold">Edit value {{ money($total) }}@if($mrp > $total) <span class="text-white/70 strike">{{ money($mrp) }}</span>@endif</span>
            </div>
        </div>
    </div>

    @if($collection->body)<div class="prose-h mx-auto mt-8 max-w-3xl text-sm">{!! $collection->body !!}</div>@endif
    ...
    @if($related->isNotEmpty())
        <div class="mt-10">
            <x-section-head title="More edits" :href="route('collections.index')" />
            <div class="grid gap-3 sm:grid-cols-3">
                @foreach($related as $c)
                    <a href="{{ $c->url }}" class="card card-hover flex items-center gap-3 p-3">
                        <span class="h-16 w-16 shrink-0 overflow-hidden rounded-lg bg-cream">@if($c->image_url)<img src="{{ $c->image_url }}" alt="" class="img-cover" loading="lazy">@endif</span>
                        <span><span class="block text-sm font-bold">{{ $c->name }}</span><span class="line-clamp-2 text-xs text-muted">{{ $c->description }}</span></span>
                    </a>
                @endforeach
            </div>
        </div>
