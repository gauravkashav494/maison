...
<section class="h-container py-6" x-data="checkout({ shipping: @js($old('shipping_method', $cart['shipping_method']['code'])), payment: @js($old('payment_method', array_key_first($paymentMethods))), codFee: {{ $codFee }} })">
    @if($errors->any())
    ...
    <form method="post" action="{{ route('checkout.place') }}" class="grid gap-6 lg:grid-cols-12 lg:gap-8" @submit="submitting = true">
        ...
        <div class="space-y-4 lg:col-span-8">
            ...
            <fieldset class="card p-4 sm:p-5">
                ...
                <div class="flex items-center gap-3"><span class="step-num">1</span><h2 class="text-base font-semibold">Contact details</h2></div>
                @guest<p class="mt-2 text-xs text-muted">Have an account? <a href="{{ route('login') }}?redirect=checkout" class="font-semibold text-red underline underline-offset-2">Log in</a> to use your saved addresses.</p>@endguest
            ...
            <fieldset class="card p-4 sm:p-5">
                ...
                <div class="flex items-center gap-3"><span class="step-num">2</span><h2 class="text-base font-semibold">Delivery address</h2></div>
                <div class="mt-4 grid gap-4 sm:grid-cols-2">
            ...
            <fieldset class="card p-4 sm:p-5">
                ...
                <div class="flex items-center gap-3"><span class="step-num">3</span><h2 class="text-base font-semibold">Delivery option</h2></div>
                <div class="mt-4 grid gap-2">
                    @foreach($shippingMethods as $m)
                        @php $free = $m['free_over'] !== null && $m['free_over'] > 0 && $cart['subtotal'] >= $m['free_over']; @endphp
                        <label class="flex cursor-pointer items-start gap-3 rounded-xl border px-4 py-3 transition-colors" :class="shipping === @js($m['code']) ? 'border-red bg-cream' : 'border-line hover:border-line'">
                            ...
                            <span class="flex-1">
                                <span class="flex items-baseline justify-between gap-3"><span class="text-sm font-bold">{{ $m['name'] }}</span><span class="text-sm font-bold tabular {{ ($free || $m['cost'] === 0) ? 'text-red' : '' }}">{{ ($free || $m['cost'] === 0) ? 'FREE' : money($m['cost']) }}</span></span>
                                <span class="mt-0.5 block text-xs text-muted">{{ $m['description'] }}@if($m['eta']) · {{ $m['eta'] }}@endif</span>
                            </span>
                        </label>
                    @endforeach
                </div>
            ...
            <fieldset class="card p-4 sm:p-5">
                ...
                <div class="flex items-center gap-3"><span class="step-num">4</span><h2 class="text-base font-semibold">Payment method</h2></div>
                <p class="mt-2 flex items-center gap-1.5 text-xs text-muted"><x-ico name="lock" :size="12" /> 100% secure. We never store card details.</p>
                <div class="mt-4 grid gap-2">
                    @foreach($paymentMethods as $code => $name)
                        <div class="rounded-xl border transition-colors" :class="payment === @js($code) ? 'border-red' : 'border-line'">
                            ...
                            <div x-show="payment === @js($code)" x-collapse x-cloak class="border-t border-line bg-cream px-4 py-4 text-sm">
                                @switch($code)
                                    @case('card')
                                        ...
                                        <p class="mt-2 text-xs text-muted">Card details are captured by the payment gateway on the next step.</p>
                                        @break
                                    @case('upi')
                                        <label><span class="label">UPI ID</span><input placeholder="name@upi" class="field"></label>
                                        <p class="mt-2 text-xs text-muted">You’ll get a collect request in your UPI app to approve.</p>
                                        @break
                                    @case('wallet') <p class="text-muted">You’ll be redirected to your wallet to authorise the payment.</p> @break
                                    @case('netbanking')
                                        ...
                                        @break
                                    @case('cod') <p class="text-muted">Pay in cash or by UPI to the delivery partner at your door.@if($codFee > 0) A handling fee of {{ money($codFee) }} applies.@endif</p> @break
                                @endswitch
                            </div>
                        </div>
                    @endforeach
        ...
        <aside class="lg:col-span-4">
            <div class="card p-4 sm:p-5 lg:sticky lg:top-[7.5rem]">
                <p class="text-sm font-semibold">Order summary <span class="font-semibold text-muted">({{ $cart['count'] }} {{ Str::plural('item', $cart['count']) }})</span></p>
                <ul class="mt-3 max-h-64 divide-y divide-line-soft overflow-y-auto">
                    @foreach($cart['items'] as $item)
                        <li class="flex items-center gap-3 py-2.5">
                            <div class="relative h-12 w-12 shrink-0 overflow-hidden rounded-lg border border-line bg-cream"><img src="{{ $item['product']['images'][0] ?? '' }}" alt="" class="img-cover"><span class="absolute -right-0 -top-0 grid h-4 min-w-4 place-items-center rounded-bl-md bg-ink px-1 text-[0.625rem] font-bold text-white">{{ $item['qty'] }}</span></div>
                            <div class="min-w-0 flex-1"><p class="truncate text-xs font-bold">{{ $item['product']['name'] }}</p><p class="text-[0.6875rem] text-muted">{{ $item['size'] }}</p></div>
                ...
                <dl class="mt-3 space-y-2 border-t border-line pt-3 text-sm" x-data>
                    <div class="flex justify-between"><dt class="text-muted">Item total</dt><dd class="tabular" x-text="$store.cart.subtotal_formatted || @js($cart['subtotal_formatted'])"></dd></div>
                    <div class="flex justify-between" x-show="$store.cart.discount > 0"><dt class="text-muted">Coupon <span x-text="$store.cart.coupon ? '(' + $store.cart.coupon.code + ')' : ''"></span></dt><dd class="tabular text-red" x-text="'− ' + $store.cart.discount_formatted"></dd></div>
                    <div class="flex justify-between"><dt class="text-muted">Delivery</dt><dd class="tabular" x-text="($store.cart.loaded ? $store.cart.shipping : @js($cart['shipping'])) === 0 ? 'FREE' : ($store.cart.shipping_formatted || @js($cart['shipping_formatted']))"></dd></div>
                    <div class="flex justify-between" x-show="codExtra > 0"><dt class="text-muted">COD handling fee</dt><dd class="tabular" x-text="fmt(codExtra)"></dd></div>
                    <div class="flex justify-between"><dt class="text-muted" x-text="'Taxes (' + ($store.cart.tax_label || @js($cart['tax_label'])) + ')'"></dt><dd class="tabular" x-text="$store.cart.tax_formatted || @js($cart['tax_formatted'])"></dd></div>
                    <div class="flex justify-between border-t border-line pt-3 text-base font-semibold"><dt>To pay</dt><dd class="tabular" x-text="fmt(($store.cart.loaded ? $store.cart.total : {{ $cart['total'] }}) + codExtra)"></dd></div>
                </dl>
                ...
                <p class="mt-3 text-center text-[0.6875rem] leading-relaxed text-muted">By placing your order you agree to our <a href="/terms" class="underline">Terms</a> and <a href="/privacy" class="underline">Privacy Policy</a>.</p>
            </div>
        </aside>
    </form>
</section>
