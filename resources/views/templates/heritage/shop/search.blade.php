...
<section class="h-container py-6">
    <form action="{{ route('search') }}" method="get" role="search" class="relative mx-auto max-w-2xl">
        <x-ico name="search" :size="18" class="pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 text-muted" />
    ...
    @if($q === '')
        <div class="mx-auto mt-6 max-w-2xl">
            <p class="text-xs font-bold uppercase tracking-wider text-muted">Popular searches</p>
    ...
    @else
        ...
        @if($products->hasPages())<div class="mt-8">{{ $products->links('vendor.pagination.heritage') }}</div>@endif
    @endif
</section>
@endsection
